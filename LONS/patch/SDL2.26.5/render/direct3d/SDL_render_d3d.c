...
SetDrawState(D3D_RenderData *data, const SDL_RenderCommand *cmd)
{
    ...
    if (texture != data->drawstate.texture) {
        ...
        //LONS add it,check has user shader
        //must set SDL_HAVE_YUV
        char *usedata = (char*)SDL_GetTextureUserData(texture);
        if (usedata) {
            usedata[3] = 0;
            //Check if it has been compiled
            int sid = usedata[0] - 1 + SHADER_LONS_USER1;
            if (usedata[2] == 0) {
                //HRESULT result = D3D9_CreatePixelShader(data->device, (D3D9_Shader)sid, &data->shaders[sid]);
                HRESULT ret1 = IDirect3DDevice9_CreatePixelShader(data->device, (DWORD*)(usedata + 4), &data->shaders[sid]);
                if (FAILED(ret1)) usedata[2] = 1;
                else usedata[2] = 2;
            }
            //compiled sucess
            if (usedata[2] > 1) {
                shader = data->shaders[sid];
                usedata[3] = 1;
            }
        }
        //LONS add end
...
D3D_CreateRenderer(SDL_Window * window, Uint32 flags)
{
    ...
    if (caps.MaxSimultaneousTextures >= 3) {
        int i;
        for (i = 0; i < SDL_arraysize(data->shaders); ++i) {
            if (i >= SHADER_LONS_USER1) 
                break;  //lons add, only running use shader.
